...
export function getEmbedUrl(url: string): string {
  ...
  if (youtubeWatchMatch) {
    return `https://www.youtube-nocookie.com/embed/${youtubeWatchMatch[1]}?rel=0`;
  }

  // Already a YouTube embed URL — normalize to nocookie domain to avoid redirect loops
  if (trimmedUrl.includes('youtube.com/embed/')) {
    return trimmedUrl.replace('://www.youtube.com/embed/', '://www.youtube-nocookie.com/embed/').replace('://youtube.com/embed/', '://www.youtube-nocookie.com/embed/');
  }
...
export function getGoogleDriveFileId(url: string): string | null {
  if (!url) return null;
  const trimmedUrl = url.trim();
  const fileMatch = trimmedUrl.match(/drive\.google\.com\/file\/d\/([a-zA-Z0-9_-]+)/);
...
export function getGoogleDriveVideoSource(url: string): string {
