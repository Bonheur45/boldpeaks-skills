...
export default function ProgramDetail() {
  ...
  const [openGroupings, setOpenGroupings] = useState<string[]>([]);

  const scrollKey = `program-scroll-${programId}`;
  const openKey = `program-open-${programId}`;
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) setOpenGroupings(JSON.parse(saved));
    } catch {}
  }, [programId, user?.id]);

  // Persist scroll position so back-nav restores it
  useEffect(() => {
    const onScroll = () => {
      ...
      <div className="p-6 lg:px-16 xl:px-24 lg:py-8 space-y-8">
        ...
        <div className="space-y-6">
          ...
          {groupings.length > 0 ? (
            <Accordion
              type="multiple"
              value={openGroupings.length ? openGroupings : groupings.map((g) => g.id)}
              onValueChange={(v) => {
                setOpenGroupings(v);
                sessionStorage.setItem(openKey, JSON.stringify(v));
              }}
              className="space-y-4 lg:max-w-[75%] lg:mx-auto"
            >
