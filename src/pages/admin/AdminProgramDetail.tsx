...
import { restoreWindowScroll, useScrollRestoration } from '@/hooks/useScrollRestoration';
...
export default function AdminProgramDetail() {
  ...
  const [highlightedMove, setHighlightedMove] = useState<{ id: string; type: 'section' | 'lesson' } | null>(null);
  ...
  useScrollRestoration(`admin-program-scroll-${programId}`, !isLoading);
  ...
  const handleSaveLesson = async () => {
    ...
    sessionStorage.setItem(`admin-program-scroll-${programId}`, String(savedScrollY));
    setIsSaving(true);
    try {
      if (editingLesson) {
  ...
  const triggerMoveFeedback = (id: string, type: 'section' | 'lesson') => {
    setHighlightedMove({ id, type });
    window.setTimeout(() => setHighlightedMove(null), 1200);
    toast({ title: type === 'section' ? 'Section moved' : 'Lesson moved' });
  };

  const moveGrouping = async (groupingId: string, direction: 'up' | 'down') => {
    const sorted = sortByOrder(groupings) as Grouping[];
    const index = sorted.findIndex((item) => item.id === groupingId);
    if (index === -1) return;
    const targetIndex = direction === 'up' ? index - 1 : index + 1;
    if (targetIndex < 0 || targetIndex >= sorted.length) return;

    const nextGroupings = [...sorted];
    [nextGroupings[index], nextGroupings[targetIndex]] = [nextGroupings[targetIndex], nextGroupings[index]];
    const updatedGroupings: Grouping[] = nextGroupings.map((group, idx) => ({ ...group, sort_order: idx + 1 }));
    setGroupings(updatedGroupings);

    try {
      await Promise.all(
        updatedGroupings.map((group) =>
          supabase.from('groupings').update({ sort_order: group.sort_order }).eq('id', group.id)
        )
      );
      triggerMoveFeedback(groupingId, 'section');
      await fetchProgramData();
    } catch (error: any) {
      toast({ variant: 'destructive', title: 'Error updating section order', description: error.message });
      fetchProgramData();
    }
  };

  const getLessonsForGrouping = (groupingId: string | null) => {
    return sortByOrder(lessons.filter((l) => l.grouping_id === groupingId));
  };

  const moveLesson = async (lessonId: string, direction: 'up' | 'down') => {
    const lesson = lessons.find((l) => l.id === lessonId);
    if (!lesson) return;
    const groupId = lesson.grouping_id;
    const sortedLessons = getLessonsForGrouping(groupId) as Lesson[];
    const index = sortedLessons.findIndex((item) => item.id === lessonId);
    if (index === -1) return;
    const targetIndex = direction === 'up' ? index - 1 : index + 1;
    ...
    const updatedLessons: Lesson[] = nextLessons.map((lesson, idx) => ({ ...lesson, sort_order: idx + 1 }));
    const updatedLessonMap = new Map(updatedLessons.map((lesson) => [lesson.id, lesson]));

    setLessons((currentLessons) =>
      sortByOrder(
        currentLessons.map((existing) => updatedLessonMap.get(existing.id) ?? existing)
      )
    );
    ...
    try {
      await Promise.all(
        updatedLessons.map((lesson) =>
          supabase.from('lessons').update({ sort_order: lesson.sort_order }).eq('id', lesson.id)
        )
      );
      triggerMoveFeedback(lessonId, 'lesson');
      await fetchProgramData();
    } catch (error: any) {
      toast({ variant: 'destructive', title: 'Error updating lesson order', description: error.message });
      fetchProgramData();
    }
  };
  ...
  const sortedGroupings = sortByOrder(groupings);
  ...
  return (
    <DashboardLayout>
      <div className="p-6 lg:p-8 space-y-8">
        {/* Header */}
        <div className="sticky top-4 z-20 rounded-3xl border border-border/70 bg-background/95 p-4 shadow-sm backdrop-blur-xl">
        ...
        <div className="space-y-6">
          {sortedGroupings.map((grouping) => (
            <Card
              key={grouping.id}
              className={`card-elevated transition-all ${highlightedMove?.id === grouping.id && highlightedMove?.type === 'section' ? 'border-primary bg-primary/5 shadow-sm' : ''}`}
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1">
                    <Button
                      variant="outline"
                      size="icon"
                      onClick={() => moveGrouping(grouping.id, 'up')}
                      disabled={groupings.findIndex((g) => g.id === grouping.id) === 0}
                    ...
                    <Button
                      variant="outline"
                      size="icon"
                      onClick={() => moveGrouping(grouping.id, 'down')}
                      disabled={groupings.findIndex((g) => g.id === grouping.id) === groupings.length - 1}
              ...
              <CardContent className="space-y-2">
                {getLessonsForGrouping(grouping.id).map((lesson, index) => (
                  <LessonRow
                    key={lesson.id}
                    lesson={lesson}
                    programId={programId!}
                    ...
                    isFirst={index === 0}
                    isLast={index === getLessonsForGrouping(grouping.id).length - 1}
                    isHighlighted={highlightedMove?.id === lesson.id && highlightedMove?.type === 'lesson'}
                  />
                ))}
              ...
              <CardContent className="space-y-2">
                {ungroupedLessons.map((lesson, index) => (
                  <LessonRow
                    ...
                    onMoveDown={() => moveLesson(lesson.id, 'down')}
                    isFirst={index === 0}
                    isLast={index === ungroupedLessons.length - 1}
                    isHighlighted={highlightedMove?.id === lesson.id && highlightedMove?.type === 'lesson'}
                  />
                ))}
              </CardContent>
...
function LessonRow({ 
  ...
  return (
    <div className={`flex items-center gap-3 p-3 rounded-lg border transition-all ${isHighlighted ? 'border-primary bg-primary/10 shadow-sm' : 'hover:bg-muted/50'}`}>
      <div className="flex items-center gap-1">
        <Button
          variant="outline"
          size="icon"
